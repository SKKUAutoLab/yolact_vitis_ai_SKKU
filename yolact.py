...
class FPN(ScriptModuleWrapper):
    ...
    def forward(self, convouts:List[torch.Tensor]):
        ...
        for lat_layer in self.lat_layers:
            ...
            if j < len(convouts) - 1:
                # Get target size from the current convolution output
                target_h, target_w = convouts[j].size()[2:]
                
                # Use direct resize to the target size to avoid mismatches
        ...
        if self.use_conv_downsample:
            ...
            for idx in range(self.num_downsample):
                # Simple max pooling for downsampling
                out.append(nn.functional.max_pool2d(out[-1], kernel_size=2, stride=2))
